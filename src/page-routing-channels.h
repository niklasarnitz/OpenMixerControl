    ...
    public:
		...
		void OnInit() override
		{
			...
			for (uint8_t i=0; i < 40; i++)
			{
				...
				lv_table_set_cell_value(objects.table_routing_dsp_input, i, 0, config->GetParameter(ROUTING_DSP_CHANNEL)->GetFormatedValue(i).c_str());
				lv_table_set_cell_value(objects.table_routing_dsp_input, i, 2, config->GetParameter(ROUTING_DSP_CHANNEL_TAPPOINT)->GetFormatedValue(i).c_str());
		...
		void OnShow() override {
			...
			custom_encoder[DISPLAY_ENCODER_4].label = "\xEF\x81\xB7 Destination \xEF\x81\xB8";
		...
		void OnChange(bool force_update) override {
			...
			if(config->HasParametersChanged({ROUTING_DSP_CHANNEL, ROUTING_DSP_CHANNEL_TAPPOINT}) || force_update)
			{
				for(auto const& index : config->GetChangedParameterIndexes({ROUTING_DSP_CHANNEL, ROUTING_DSP_CHANNEL_TAPPOINT}))
                {
					lv_table_set_cell_value(objects.table_routing_dsp_input, index, 0, config->GetParameter(ROUTING_DSP_CHANNEL)->GetFormatedValue(index).c_str());
					lv_table_set_cell_value(objects.table_routing_dsp_input, index, 2, config->GetParameter(ROUTING_DSP_CHANNEL_TAPPOINT)->GetFormatedValue(index).c_str());
				}
			}
		}
        ...
        bool OnDisplayEncoderTurned(X32_ENC encoder, int amount) override {
            switch (encoder){
				...
				case X32_ENC_ENCODER1:
					config->Change(ROUTING_DSP_CHANNEL, amount, gui_selected_item);
					break;
				case X32_ENC_ENCODER2:
					...
					for (uint8_t i=gui_selected_item; i<(gui_selected_item+8); i++) {
						config->Change(ROUTING_DSP_CHANNEL, amount, i);
					}
					break;
				case X32_ENC_ENCODER3:
					config->Change(ROUTING_DSP_CHANNEL_TAPPOINT, amount, gui_selected_item);
					break;
