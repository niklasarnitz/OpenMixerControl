...
int artnet_net_init(node n, const char *preferred_ip) {
    ...
    // Wir erstellen einen temporären Socket für die IOCTL-Abfragen
    fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (fd < 0) {
        artnet_error("Socket creation failed for eth0 lookup");
        return ARTNET_ENET;
    }

    strncpy(ifr.ifr_name, "eth0", IFNAMSIZ-1);

    // 1. IP-Adresse von eth0 abfragen
    if (ioctl(fd, SIOCGIFADDR, &ifr) == 0) {
        n->state.ip_addr = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr;
    } else {
        artnet_error("Could not get IP for eth0");
        close(fd);
        return ARTNET_ENET;
    }

    // 2. Broadcast-Adresse abfragen
    if (ioctl(fd, SIOCGIFBRDADDR, &ifr) == 0) {
        n->state.bcast_addr = ((struct sockaddr_in *)&ifr.ifr_broadaddr)->sin_addr;
    } else {
        // Fallback: berechne einfache Broadcast-Adresse (Standard für viele Netze)
        n->state.bcast_addr.s_addr = n->state.ip_addr.s_addr | htonl(0x000000FF);
    }

    // 3. MAC-Adresse (Hardware-Adresse) abfragen
    if (ioctl(fd, SIOCGIFHWADDR, &ifr) == 0) {
        memcpy(&n->state.hw_addr, ifr.ifr_hwaddr.sa_data, ARTNET_MAC_SIZE);
    } else {
        // Fallback: Dummy MAC, falls IOCTL fehlschlägt
        memset(&n->state.hw_addr, 0, ARTNET_MAC_SIZE);
    }

    close(fd);

    if (n->state.verbose) {
        printf("#### FORCED INTERFACE eth0 ####\n");
